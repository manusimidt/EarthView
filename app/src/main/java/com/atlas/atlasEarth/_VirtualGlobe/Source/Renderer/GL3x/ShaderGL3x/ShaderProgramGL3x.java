...
public abstract class ShaderProgramGL3x extends ShaderProgramNameGL3x {
    ...
    public ShaderProgramGL3x(Context context, int vsRawID, int fsRawID) {
        ...
        vsID = new ShaderObjectGL3x(context, GLES31.GL_VERTEX_SHADER, vsRawID).getShaderID();
        fsID = new ShaderObjectGL3x(context, GLES31.GL_FRAGMENT_SHADER, fsRawID).getShaderID();
    ...
    protected void loadBoolean(int location, boolean value) {
        if (value) {
            GLES31.glUniform1f(location, 1);
        } else {
            GLES31.glUniform1f(location, 0);
        }
    }
