...
public class Post extends Renderable {
    ...
    private void calculateMesh(Context context) {
        ...
        /*TESTING FOR SECOND MESH CONSTRUCTOR*/
        int indicesArray[] = new int[indices.size() * 3];
        float positionArray[] = new float[positions.size() * 3];
        float normalArray[] = new float[normals.size() * 3];
        float textureArray[] = new float[textureCoords.size() * 2];

        int dimensionIterator = 0;
        for (int i = 0; i <= indices.size() - 1; i++) {
            indicesArray[dimensionIterator++] = indices.get(i).getT0();
            indicesArray[dimensionIterator++] = indices.get(i).getT1();
            indicesArray[dimensionIterator++] = indices.get(i).getT2();
        }
        dimensionIterator = 0;
        for (int i = 0; i <= positions.size() - 1; i++) {
            positionArray[dimensionIterator++] = positions.get(i).x;
            positionArray[dimensionIterator++] = positions.get(i).y;
            positionArray[dimensionIterator++] = positions.get(i).z;
        }
        dimensionIterator = 0;
        for (int i = 0; i <= normals.size() - 1; i++) {
            normalArray[dimensionIterator++] = normals.get(i).x;
            normalArray[dimensionIterator++] = normals.get(i).y;
            normalArray[dimensionIterator++] = normals.get(i).z;
        }
        dimensionIterator = 0;
        for (int i = 0; i <= textureCoords.size() - 1; i++) {
            textureArray[dimensionIterator++] = textureCoords.get(i).x;
            textureArray[dimensionIterator++] = textureCoords.get(i).y;
        }
        super.mesh = new Mesh(indicesArray, positionArray, normalArray, textureArray);
